...
type ('a,'b) token
type svalue
val ASSOCINDICATOR: ({ cut:bool,strength:int,direction:FormatTemplate.assocDirection } ) *  'a * 'a -> (svalue,'a) token
val FORMATINDICATOR: ({ space:bool,newline:{ priority:FormatTemplate.priority }  option } ) *  'a * 'a -> (svalue,'a) token
